"""Config flow to configure the Decora WiFi integration."""
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a DecoraWiFi config flow."""
    ...
    async def _async_verify(self, step_id: str, schema: vol.Schema) -> FlowResult:
        ...
        errors = {}
        try:
            if DecoraWiFiSession().login(self._username, self._password) is None:
                errors["base"] = "invalid_auth"
        # disable broad-except because the underlying library doesn't handle exceptions itself
        except Exception as err:  # pylint: disable=broad-except
            LOGGER.error("Unknown Decora WiFi error: %s", err)
            errors["base"] = "unknown"
